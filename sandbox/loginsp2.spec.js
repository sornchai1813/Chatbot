...
test('Sprinklr Full Process - Search & Assert #1234', async ({ browser }) => {
  ...
  // 4. ปิด Dismiss Popup (กันหน้าต่าง Search วูบ)
  try {
  ...
  // 5. Logic: เปิด Search (ถ้าหน้าจอว่าง)
  ...
  const caseRow = page.locator('div').filter({ hasText: /^AAphinat Hlongkaow2688431780071#1234\+2Jul 08$/ }).nth(5);
  ...
  await expect(page.locator('#container-for-CUSTOM_FIELD_5098')).toContainText('Select value');
  await expect(page.locator('#container-for-CUSTOM_FIELD_5099')).toContainText('Select value');
