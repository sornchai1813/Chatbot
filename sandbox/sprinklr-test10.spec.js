...
const runSetGroupInSeparateContext = async ({ browser, setGroup, questionsToRun, loadedFrom, roundLabel, slotLabel }) => {
  ...
  const keepOpenOnError = process.env.KEEP_OPEN_ON_ERROR !== 'false';
  ...
  const runDd = String(runNow.getDate()).padStart(2, '0');
  const runMm = String(runNow.getMonth() + 1).padStart(2, '0');
  const runYy = String(runNow.getFullYear()).slice(-2);
  const dateSuffix = `${runDd}${runMm}${runYy}`;
  ...
  const resolveCaseNumber = async () => {
    const fallback = `ErrorCase_${Date.now()}`;
    if (!frame) return fallback;

    try {
      const caseIdText = await frame.locator('text=/หมายเลขเคส/').last().innerText({ timeout: 3000 });
      const extracted = String(caseIdText || '').replace(/[^0-9]/g, '');
  ...
  try {
    ...
    frame = page.frameLocator('iframe[name="spr-chat__box-frame"]');
    realFrame = page.frame({ name: 'spr-chat__box-frame' });
    ...
    const errorCaseNumber = await resolveCaseNumber();
    const errorCsvName = `Log_${errorCaseNumber}_${outputGroupTag}_${dateSuffix}_error.csv`;
    const errorCsvPath = path.join(SCREENSHOT_DIR, errorCsvName);
    const errorPngName = `${errorCaseNumber}_${outputGroupTag}_${dateSuffix}_error.png`;
    const errorPngPath = path.join(SCREENSHOT_DIR, errorPngName);

    try {
      let errorCsvContent = `\ufeff#หมายเลขเคส: ${errorCaseNumber}\r\n#setGroup: ${setGroup}\r\n#status: error\r\n#error: ${escapeCsvField(error?.message || 'unknown error')}\r\n\r\n`;
      qaResults.forEach((res, index) => {
        const q = escapeCsvField(res.q);
        const a = escapeCsvField(res.a);
        errorCsvContent += `Q${index + 1}: "${q}"\r\nA${index + 1}: "${a}"\r\n\r\n`;
      });
      fs.writeFileSync(errorCsvPath, errorCsvContent);
    } catch (csvError) {
      console.log(`⚠️ ${roundLabel} ${slotLabel}: เขียน error csv ไม่สำเร็จ ${setGroup} | ${csvError.message}`);
    }

    try {
      await page.screenshot({ path: errorPngPath, fullPage: true, animations: 'disabled' });
      console.log(`🧯 ${roundLabel} ${slotLabel}: เกิดข้อผิดพลาด ${setGroup} | error csv: ${errorCsvName} | error screenshot: ${errorPngName}`);
    } catch (shotError) {
      console.log(`⚠️ ${roundLabel} ${slotLabel}: แคป error screenshot ไม่สำเร็จ ${setGroup} | ${shotError.message}`);
    }

    if (keepOpenOnError) {
      console.log(`🛑 ${roundLabel} ${slotLabel}: ค้างหน้าเว็บไว้เพราะ error (${setGroup}) | ปิดได้เมื่อกด Resume/Continue ใน Playwright Inspector`);
    ...
    if (!encounteredError || !keepOpenOnError) {
