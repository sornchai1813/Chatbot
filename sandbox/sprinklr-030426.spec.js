...
const csvRowsToObjects = (rows = []) => {
  ...
  return rows.slice(1).map((cells) => {
    ...
    obj.assertType = obj.asserttype || obj.assert_type || '';
    obj.expected = obj.expected || obj.expect || '';
...
const setGroup = runConfig.setGroup; 

test('CLICX Chatbot Automation - Stable v2.0', async ({ page }, testInfo) => {
  const data = sessions[0]; 
  let questionsToRun = [];
  let loadedFrom = 'sheet';

  try {
    questionsToRun = await loadQuestionsFromSheet(setGroup);
    console.log(`📄 โหลดคำถามจาก Google Sheet สำเร็จ: ${questionsToRun.length} ข้อ`);

    if (!questionsToRun.length) {
      throw new Error(`ไม่พบคำถามในชีตสำหรับ setGroup = ${setGroup}`);
    }
  } catch (error) {
    questionsToRun = data[setGroup] || [];
    loadedFrom = 'local';
    console.log(`⚠️ โหลดจาก Google Sheet ไม่ได้ ใช้ local fallback แทน: ${error.message}`);
  }
  ...
  if (!questionsToRun.length) {
    throw new Error(`❌ ไม่พบชุดคำถาม "${setGroup}" ทั้งใน Google Sheet และ local data`);
  }

  console.log(`📚 ใช้คำถามจาก: ${loadedFrom} | setGroup=${setGroup} | จำนวน=${questionsToRun.length}`);
  ...
  console.log(`🚀 ${roundLabel}: เริ่มรันชุดคำถาม ${setGroup}`);
  ...
  console.log(`🔢 ${roundLabel}: case ${caseNumber}`);

  const _now = new Date();
  const _dd = String(_now.getDate()).padStart(2, '0');
  const _mm = String(_now.getMonth() + 1).padStart(2, '0');
  const _yy = String(_now.getFullYear()).slice(-2);
  const dateSuffix = `${_dd}${_mm}${_yy}`;

  // บันทึก CSV
  const csvPath = path.join(SCREENSHOT_DIR, `Log_${caseNumber}_${setGroup}_${dateSuffix}.csv`);
  ...
  const finalPath = path.join(SCREENSHOT_DIR, `${caseNumber}_${setGroup}_${dateSuffix}.png`);
  const chatBody = realFrame.locator('body');
  await chatBody.screenshot({ path: finalPath, animations: 'disabled' });

  console.log(`🏁 ${roundLabel}: เสร็จสิ้น case ${caseNumber} | CSV: Log_${caseNumber}_${setGroup}_${dateSuffix}.csv | PNG: ${caseNumber}_${setGroup}_${dateSuffix}.png`);
  await page.pause(); 
});
